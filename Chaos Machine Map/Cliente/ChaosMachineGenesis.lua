...
---------------------------------------
---------COMBINA�OES EM MAPAS----------
---------------------------------------
CHAOS_MACHINE_CONFIG_COMB_SELECTION[0] = {--CRIA�OES POR MAPA [NUMERO MAP] --LORENCIA
	{ Combination = 1,   CombinationSuccessRate = 100.00, CombinationName = "Jewel of Cristal Red"},
}

CHAOS_MACHINE_CONFIG_COMB_SELECTION[3] = {--CRIA�OES POR MAPA [NUMERO MAP] --NORIA
...
-----------------------------------------------------
CHAOS_MACHINE_CONFIG_COMB_INGREDIENTS[3] = { --IGREDIENTES DAS CRIA��ES POR MAPA = NORIA
	[1] = {-- NUMERO DA CRIA��O DO MAP
...
[11] = "Renew de la m�quina del caos",
...
function RenderTextPageOne(x , y)
	...
	for key = 1, #CHAOS_MACHINE_CONFIG_COMB_SELECTION[UserGetMap()] do
		if line >= ChaosMachineScrollBarCurrentLine and line < ChaosMachineScrollBarMaxRenderLine
		then
			local combs = CHAOS_MACHINE_CONFIG_COMB_SELECTION[UserGetMap()][key]
...
function ChaosMachineRenderCombinationPage()
	...
	RenderText3(m_Pos.x, m_Pos.y + 40, string.format('%s', CHAOS_MACHINE_CONFIG_COMB_SELECTION[UserGetMap()][ChaosMachineSelectedKey].CombinationName), 190, 3)
	...
	then
		...
		RenderText3(m_Pos.x, m_Pos.y + 70, string.format(CHAOS_MACHINE_TEXTS[GetLanguage()][7], CHAOS_MACHINE_CONFIG_COMB_SELECTION[UserGetMap()][ChaosMachineSelectedKey].CombinationSuccessRate), 190, 3)
		...
		then
			...
			then
				...
				then
					if CheckPressedKey(Keys.LButton) == 1
					then
						ChaosMachineSelectedCombSelected = CHAOS_MACHINE_CONFIG_COMB_SELECTION[UserGetMap()][key].Combination
...
function ChaosMachineSetIngredientItens()
	...
	if CHAOS_MACHINE_CONFIG_COMB_SELECTION[UserGetMap()][ChaosMachineSelectedKey] == nil
	then
		ChaosMachineSelectedKey = 1
		return
	end
	
	if CHAOS_MACHINE_CONFIG_COMB_INGREDIENTS[UserGetMap()][ChaosMachineSelectedCombSelected] == nil
	then
		return
	end
	...
	for i in ipairs(CHAOS_MACHINE_CONFIG_COMB_INGREDIENTS[UserGetMap()][ChaosMachineSelectedCombSelected]) do
		ChaosMachineItensIngredients[value] = { Ingredients = CHAOS_MACHINE_CONFIG_COMB_INGREDIENTS[UserGetMap()][ChaosMachineSelectedCombSelected][i] }
...
function ChaosMachineOpenning()
	...
	ChaosMachineScrollBarPosY = 0
	ChaosMachineScrollBarPosMouse = 0
	ScrollBarPageMin = #CHAOS_MACHINE_CONFIG_COMB_SELECTION[UserGetMap()]
	ChaosMachineScrollBarPosYMultiplier = (160 / (#CHAOS_MACHINE_CONFIG_COMB_SELECTION[UserGetMap()] - ChaosMachineScrollBarMaxRenderLine))
